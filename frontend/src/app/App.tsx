...
export default function App() {
  ...
  const isAuthChecked = authContenxt.state.isAuthChecked
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => setSplashVisible(false), 300)
  ...
  return (
    <>
          ...
          <Routes>
            <Route path="/" element={authContenxt.state.isAuthenticated ? <Layout /> : <Pages.LoginPage />}>
              ...
              >
                ...
                <Route
                  path="delivery"
                  element={<ProtectedRoute children={<Pages.DeliverySettingsPage />} permissions={['settings.page']} />}
                />
                <Route
                  path="diagnostics"
                  element={<ProtectedRoute children={<Pages.DiagnosticsSettingsPage />} permissions={['settings.page']} />}
                />
              </Route>
      ...
      {splashVisible && (
        <div
          className={`fixed inset-0 z-50 flex items-center justify-center bg-background transition-opacity duration-300 ease-out ${isAuthChecked ? 'opacity-0' : 'opacity-100'}`}
        >
          <div className="flex animate-in fade-in-0 zoom-in-95 items-center gap-3 duration-500">
