...
export function DataTable() {
  ...
  const { removeWarehouseTransaction } = useWarehouseTransactionContext()
  ...
  const { data: { warehouseTransactions = [], warehouseTransactionsCount = 0 } = {}, isLoading, isFetching } = useWarehouseTransactionQuery(
    { pagination, filters, sorters },
    { options: {
      select: response => ({
        warehouseTransactions: response.data.warehouseTransactions,
        warehouseTransactionsCount: response.data.warehouseTransactionsCount,
      }),
  ...
  const handleBulkRemove = () => {
    const ids = warehouseTransactions.filter((_, index) => rowSelection[index]).map(item => item.id)
    removeWarehouseTransaction({ ids })
    setRowSelection({})
  }
  ...
  return (
    <>
      ...
      <TablePagination
        ...
        totalPages={Math.ceil(warehouseTransactionsCount / pagination.pageSize)}
        changePagination={changePagination}
        selectedCount={Object.keys(rowSelection).length}
        totalCount={warehouseTransactionsCount}
      />
    </>
  )
}

function SubRowItems({ transactionId, columnsLength, language }) {
  const { data, isLoading, isFetching, error } = useWarehouseTransactionItemsQuery(
    { filters: { transactionId } },
    { options: { placeholderData: prevData => prevData } },
  )

  const itemsData = data?.data?.warehouseTransactionsItems || []
