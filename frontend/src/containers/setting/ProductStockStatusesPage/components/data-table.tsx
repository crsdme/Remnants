...
import { useProductStockStatusQuery } from '@/api/hooks'
...
export function DataTable() {
  ...
  const { productStockStatuses = [], productStockStatusesCount = 0, isLoading, isFetching } = useProductStockStatusQuery(
    { pagination, filters, sorters },
    { options: { placeholderData: prevData => prevData } },
  )

  const table = useReactTable({
    data: productStockStatuses,
    columns,
    ...
    <>
      <div className="w-full flex justify-between items-start max-md:flex-col gap-2 py-2">
        ...
        <div className="flex gap-2">
          <ColumnVisibilityMenu table={table} tableId="product-stock-status" />
        </div>
      </div>
      ...
      <TablePagination
        ...
        totalPages={Math.ceil(productStockStatusesCount / pagination.pageSize)}
        changePagination={setPagination}
        totalCount={productStockStatusesCount}
      />
    </>
  )
}
