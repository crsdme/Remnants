...
import { useDeliveryServiceQuery } from '@/api/hooks'
...
export function DataTable() {
  ...
  const { removeDeliveryService } = useDeliveryServiceContext()
  ...
  const { data: { deliveryServices = [], deliveryServicesCount = 0 } = {}, isLoading, isFetching } = useDeliveryServiceQuery(
    { pagination, filters, sorters },
    { options: {
      select: response => ({
        deliveryServices: response.data.deliveryServices,
        deliveryServicesCount: response.data.deliveryServicesCount,
      }),
  ...
  const handleBulkRemove = () => {
    const ids = deliveryServices.filter((_, index) => rowSelection[index]).map(item => item.id)
    removeDeliveryService({ ids })
    setRowSelection({})
  }
  ...
  return (
    <>
      ...
      <TablePagination
        ...
        totalPages={Math.ceil(deliveryServicesCount / pagination.pageSize)}
        changePagination={changePagination}
        selectedCount={Object.keys(rowSelection).length}
        totalCount={deliveryServicesCount}
      />
    </>
  )
}
