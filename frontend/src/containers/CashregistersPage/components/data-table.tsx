...
import { useCashregisterQuery } from '@/api/hooks'
...
export function DataTable() {
  ...
  const { removeCashregister } = useCashregisterContext()
  ...
  const { data: { cashregisters = [], cashregistersCount = 0 } = {}, isLoading, isFetching } = useCashregisterQuery(
    { pagination, filters, sorters },
    { options: {
      select: response => ({
        cashregisters: response.data.cashregisters,
        cashregistersCount: response.data.cashregistersCount,
      }),
  ...
  const handleBulkRemove = () => {
    const ids = cashregisters.filter((_, index) => rowSelection[index]).map(item => item.id)
    removeCashregister({ ids })
    setRowSelection({})
  }
    ...
    <>
      <div className="w-full flex justify-between items-start max-md:flex-col gap-2 py-2">
        ...
        <div className="flex gap-2">
          ...
          />
          <ColumnVisibilityMenu table={table} tableId="cashregister" />
        </div>
      </div>
      ...
      <TablePagination
        ...
        totalPages={Math.ceil(cashregistersCount / pagination.pageSize)}
        changePagination={changePagination}
        selectedCount={Object.keys(rowSelection).length}
        totalCount={cashregistersCount}
      />
    </>
  )
}
