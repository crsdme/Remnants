...
export function DataTable() {
  ...
  const { warehouses, warehousesCount, isLoading, isFetching } = useWarehouseQuery(
  ...
  const renderRow = (row: Row<WarehouseDTO>) => (
    <Fragment key={row.id}>
      <TableRow
    ...
    <>
      <div className="w-full flex justify-between items-start max-md:flex-col gap-2 py-2">
        ...
        <div className="flex gap-2">
          <ColumnVisibilityMenu table={table} tableId="warehouse" />
        </div>
      </div>
      ...
      <TablePagination
        ...
        totalPages={Math.ceil(warehousesCount / pagination.pageSize)}
        changePagination={setPagination}
        totalCount={warehousesCount}
      />
    </>
  )
}
