...
export function BarcodeProvider({ children }: BarcodeProviderProps) {
  ...
  const useMutateCreateBarcode = useCreateBarcode({
    options: {
      onSuccess: ({ data }) => {
        closeModal()
  ...
  const useMutateEditBarcode = useEditBarcode({
    options: {
      onSuccess: ({ data }) => {
        closeModal()
        queryClient.invalidateQueries({ queryKey: ['barcodes'] })
  ...
  const useMutateRemoveBarcode = useRemoveBarcodes({
    options: {
      onSuccess: ({ data }) => {
        queryClient.invalidateQueries({ queryKey: ['barcodes'] })
        queryClient.invalidateQueries({ queryKey: ['products'] })
  ...
  const useMutateRequestBarcodes = useMutateBarcodes({
    options: {
      onSuccess: ({ data }) => {
