...
export function WarehouseProvider({ children }: WarehouseProviderProps) {
  ...
  const useMutateCreateWarehouse = useCreateWarehouse({
    options: {
      onSuccess: ({ data }) => {
        closeModal()
  ...
  const useMutateEditWarehouse = useEditWarehouse({
    options: {
      onSuccess: ({ data }) => {
        closeModal()
        queryClient.invalidateQueries({ queryKey: ['warehouses'] })
  ...
  const useMutateRemoveWarehouse = useRemoveWarehouses({
    options: {
      onSuccess: ({ data }) => {
        queryClient.invalidateQueries({ queryKey: ['warehouses'] })
        toast.success(t(`response.title.${data.code}`), { description: `${t(`response.description.${data.code}`)} ${data.description || ''}` })
