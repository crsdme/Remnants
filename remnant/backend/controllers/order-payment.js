...
const createOrderPayment = async ({ order, cashregister, cashregisterAccount, paymentStatus, amount, currency, paymentDate, comment }) => {
    let status = null;
    let data = null;
    let warnings = [];
    let errors = [];
    ...
    const createdOrderPayment = await orderPaymentModel.create({ order, cashregister, cashregisterAccount, paymentStatus, amount, currency, paymentDate, comment });
...
const editOrderPayment = async ({ _id, cashregister, cashregisterAccount, paymentStatus, amount, currency, date, comment }) => {
    let status = null;
    let data = null;
    let warnings = [];
    let errors = [];
    ...
    const editedOrderPayment = await orderPaymentModel.updateOne({ _id }, { cashregister, cashregisterAccount, paymentStatus, amount, currency, date, comment });
...
const getOrderPayments = async ({ filter, sorter, pagination }) => {
    ...
    let orderPaymentQuery = orderPaymentModel.aggregate(pipline);

    if (allPages === false || !allPages) {
        orderPaymentQuery = orderPaymentQuery.skip((current - 1) * pageSize).limit(pageSize);
    }
