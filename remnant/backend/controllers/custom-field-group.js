...
const createCustomFieldGroup = async ({ _id, names, priority, customFields }) => {
    let status = null;
    let data = null;
    let warnings = [];
    let errors = [];
    ...
    const createdCustomFieldGroup = await customFieldGroupModel.create({ _id, names, priority, customFields });
...
const editCustomFieldGroup = async ({ _id, names, priority, customFields }) => {
    ...
    const editedCustomFieldGroup = await customFieldGroupModel.findOneAndUpdate({ _id }, { names, priority, customFields }, { new: true });
...
const getCustomFieldGroups = async ({ filter, sorter, pagination }) => {
    ...
    let customFieldsGroupQuery = customFieldGroupModel.aggregate(pipline);

    if (allPages === false || !allPages) {
        customFieldsGroupQuery = customFieldsGroupQuery.skip((current - 1) * pageSize).limit(pageSize);
    }
...
const removeCustomFieldGroup = async ({ _id }) => {
    ...
    const removedCustomFieldOption = await customFieldGroupModel.deleteOne({ _id });
