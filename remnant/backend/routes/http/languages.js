...
const { createLanguage, getLanguages, removeLanguage, editLanguage, } = require('../../controllers/language.js');
...
router.post('/get', authenticateToken, async (req, res) => {
    const { status, data, errors, warnings, info } = await getLanguages(req.body);
    
    const { userId, ...params } = req.body;

    createLog({ type: 'get', route: 'languages', ip: req.socket.remoteAddress, userId, params: JSON.stringify(params) })
