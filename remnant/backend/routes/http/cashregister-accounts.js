...
const { createCashregisterAccount, removeCashregisterAccount, editCashregisterAccount, getCashregisterAccounts } = require('../../controllers/cashregister-account.js');
...
router.post('/get', authenticateToken, async (req, res) => {
    const { status, data, errors, warnings, info } = await getCashregisterAccounts(req.body);

    const { userId, ...params } = req.body;

    createLog({ type: 'get', route: 'cashregister-accounts', ip: req.socket.remoteAddress, userId, params: JSON.stringify(params) })
