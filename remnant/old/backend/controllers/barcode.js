...
const editBarcode = async ({ _id, code, products }) => {
    ...
    const editedBarcode = await barcodeModel.findOneAndUpdate({ _id }, { code, products }, { new: true });
...
const getBarcodes = async ({ filter, sorter, pagination }) => {
    ...
    let barcodesQuery = barcodeModel.aggregate(pipline);

    if (allPages === false || !allPages) {
        barcodesQuery = barcodesQuery.skip((current - 1) * pageSize).limit(pageSize);
    }
