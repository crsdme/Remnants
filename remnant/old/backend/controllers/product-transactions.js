...
const createProductTransaction = async ({ to, from, products, type, purchaseId, orderId, comment, userId }) => {
    let status = null;
    let data = null;
    let warnings = [];
    let errors = [];
...
const editProductTransaction = async ({ to, from, exchangeRate, paymentDate, comment, userId }) => {
    let status = null;
    let data = null;
    let warnings = [];
    let errors = [];
    ...
    const editedProductTransaction = await productTransactionsModel.updateOne({ _id }, { to, from, exchangeRate, paymentDate, comment });
...
const getProductTransactions = async ({ filter, sorter, pagination }) => {
    ...
    let productTransactionsQuery = productTransactionsModel.aggregate(pipline);

    if (allPages === false || !allPages) {
        productTransactionsQuery = productTransactionsQuery.skip((current - 1) * pageSize).limit(pageSize);
    }
...
const removeProductTransaction = async ({ _id }) => {
    ...
    const removedProductTransaction = await productTransactionsModel.updateOne({ _id }, { removed: true });
