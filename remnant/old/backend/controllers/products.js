...
const editProduct = async ({
  ...
  customFields = Object.entries(JSON.parse(customFields)).map(
    ([key, value]) => ({
      _id: key,
      data: value,
    })
  ...
  fileList.forEach((file) => {
    const sameFile = product.images.find(
      (image) => JSON.stringify(image._id) === JSON.stringify(file.uid)
    );
    if (sameFile) {
...
const getProducts = async ({ filters, sorter, pagination }) => {
  ...
  let productsQuery = productsModel.aggregate(pipeline);
...
const getProduct = async ({ _id }) => {
  ...
  let productQuery = productsModel.aggregate(pipeline);
...
const removeProduct = async ({ _id }) => {
  ...
  const removedProduct = await productsModel.updateOne(
    { _id },
    { removed: true }
  );
