...
const createCustomFieldOption = async ({ _id, options }) => {
    ...
    for (option of options) {
        createdOption = await customFieldOptionModel.create({ names: option.names });
        await customFieldModel.updateOne({ _id }, { $push: { options: createdOption._id } });
    }
...
const editCustomFieldOption = async ({ _id, names }) => {
    ...
    const editedCustomFieldOption = await customFieldOptionModel.findOneAndUpdate({ _id }, { names }, { new: true });
...
const getCustomFieldOptions = async ({ filter, sorter, pagination }) => {
    ...
    let customFieldsQuery = customFieldOptionModel.aggregate(pipline);
...
const removeCustomFieldOption = async ({ _id }) => {
    ...
    const removedCustomFieldOption = await customFieldOptionModel.deleteOne({ _id });
