...
const createCustomField = async ({
  ...
  const createdCustomField = await customFieldModel.create({
    names,
    model,
    is_multiple,
    type,
...
const editCustomField = async ({
  ...
  const editedCustomField = await customFieldModel.findOneAndUpdate(
    { _id },
    { names, model, is_multiple, type, priority, options },
    { new: true }
  );
...
const getCustomFields = async ({ filter, sorter, pagination }) => {
  ...
  let customFieldsQuery = customFieldModel.aggregate(pipeline);
